...
static const int32_t b4const_table[16] = {
    -1, 1, 2, 3, 4, 5, 6, 7,
    8, 10, 12, 16, 32, 64, 128, 256
};


static const uint32_t b4constu_table[16] = {
...
{
    if (insn->operand_count >= 4) return;
...
{
    int32_t offset = ((int32_t)(imm << 16)) >> 14;
    uint64_t vaddr = (((pc + 3) >> 2) << 2) + offset;
    return vaddr;
}
