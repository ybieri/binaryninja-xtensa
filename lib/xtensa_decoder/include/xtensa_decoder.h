...
{
    return (id == XTENSA_INS_RET || id == XTENSA_INS_RETW ||
            id == XTENSA_INS_RET_N || id == XTENSA_INS_RETW_N);
}


static inline bool xtensa_is_branch(XtensaInsnId id)
{
    return (id == XTENSA_INS_J || id == XTENSA_INS_JX ||
            id == XTENSA_INS_BF || id == XTENSA_INS_BT ||
...
{
    return (id == XTENSA_INS_BF || id == XTENSA_INS_BT ||
            (id >= XTENSA_INS_BEQ && id <= XTENSA_INS_BGEUI) ||
            id == XTENSA_INS_BEQZ_N || id == XTENSA_INS_BNEZ_N);
}
...
struct XtensaOperand
{
    ...
    union
    {
        uint32_t reg;
        int32_t imm;
        struct {uint32_t base; int32_t offset;} mem;
        uint64_t target;
        uint32_t sreg;
        uint64_t _r;
    };

    XtensaOperand() : type(XTENSA_OP_INVALID)
    {
        _r = 0;
    }
};
